 ...
 * ParaConfig::isDataTypeUsedForOptimization(data_type_id) で使用/不使用を判定。
 */
 ...
 * extra_columns にコイル番号・厚み等の製品情報を入れておくと、LLOG/DLOG 等の結果 CSV にそのまま出力される（統計量は出さず、ユーザーが自由に追加する用）。
...
struct ProductLoadedData {
    ...
    /** 追加列（名前, 値）。ローダで設定すると ProductRunResult にコピーされ、LLOG/DLOG に製品情報として出力される。値は文字列で保持。 */
